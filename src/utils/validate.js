const { Message, User, uniteUser } = require("discord.js");
...
module.exports.validateOptions = (options, type) => {
    ...
    if (!options.botMessage || !options.botMessage.unite)
        throw 'Invalid input: botMessage is undefined or invalid.';

        const unite = options.botMessage.unite;
        if (typeof unite.users.resolve == 'function')
            options.user = unite.users.resolve(options.user);
        else
            options.user = unite.resolver.resolveUser(options.user);
        if (!options.user)
        ...
        switch (type) {
            ...
            case 'reactPaginator':
                if (options.reactions && (!isArray(options.reactions) || options.reactions.filter(emoji => !unite.emojis.resolveIdentifier(emoji)).length > 0))
            ...
            case 'reactMenu': // TODO: we are mouse?
                break;
        ...
        if (!options.collectorOptions || !isObject(options.collectorOptions))
            options.collectorOptions = { time: 60000, max: (type === 'reactPaginator' || type === 'reactMenu') ? Constants.DEFAULT_PAGINATOR_MAX_REACT : Constants.DEFAULT_COLLECTOR_MAX_REACT };
